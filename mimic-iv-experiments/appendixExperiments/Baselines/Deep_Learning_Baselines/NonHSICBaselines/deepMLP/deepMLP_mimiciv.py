"""
DeepMLP Bag-of-Codes + Weighted Multi-Task BCE Clinical Outcome Predictor
==========================================================================

Overview
--------
This script implements a bag-of-codes MLP model (DeepMLPBagOfCodes) trained with
binary cross-entropy (BCE) on clinical admission data. ICD-10 diagnosis codes are
treated as an unordered multiset: each admission's codes are summed via an
EmbeddingBag layer, normalized, and passed through a 3-layer MLP to produce a
single scalar risk score. Training follows a two-stage weighted multi-task strategy:
Stage 1 trains per-target single-task models to derive loss weights; Stage 2 trains
one unified model on a normalized weighted multi-task BCE objective. Checkpoint
selection in both stages uses validation BCE.
...
5.  Stage 1 — Per-target single-task BCE training (10 epochs, AdamW, LR=1e-3):
        - Fresh DeepMLPBagOfCodes model per target.
...
7.  Stage 2 — Weighted multi-task BCE training (10 epochs, AdamW, LR=1e-3):
        - Single DeepMLPBagOfCodes model trained on normalized weighted masked BCE
...
Architecture: DeepMLPBagOfCodes
--------------------------------
- EmbeddingBag (vocab_size → hidden_dim=128, mode="sum"): sums token embeddings
  per admission, ignoring PAD tokens via offset-based sparse indexing
- LayerNorm applied to the summed bag representation
- MLP tower: Linear(128→128) → ReLU → Linear(128→128) → ReLU → Linear(128→1)
...
Running
-------
    python deepMLP_mimiciv.py --data_root /path/to/final_datasets

Override training seed:
    SEED=42 python deepMLP_mimiciv.py --data_root /path/to/final_datasets

Example:
    python deepMLP_mimiciv.py --data_root ./data/final_datasets
"""
...
################################################################
# MODEL: DeepMLPBagOfCodes
################################################################
class DeepMLPBagOfCodes(nn.Module):
    """
    Bag-of-codes (count vector) -> MLP -> scalar logit.
    Efficient implementation using EmbeddingBag:
      sum_{codes} W[code]  ==  Linear(multihot_or_counts)
    """
    def __init__(self, vocab_size: int, hidden_dim: int = 128):
        super().__init__()
        # Produces (B, hidden_dim) by summing "code embeddings"
        self.bag = nn.EmbeddingBag(vocab_size, hidden_dim, mode="sum", include_last_offset=False)
    ...
    def forward(self, x_ids: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        ...
        # Flatten non-pad token ids
        flat = x_ids[keep]  # (nnz,)

        # Offsets: where each example starts in `flat`
        counts = keep.sum(dim=1, dtype=torch.long)  # (B,)
        offsets = torch.zeros(B, device=x_ids.device, dtype=torch.long)
        if B > 1:
            offsets[1:] = counts.cumsum(dim=0)[:-1]

        # EmbeddingBag handles empty bags -> returns zeros for that row (good)
        pooled = self.bag(flat, offsets)  # (B, hidden_dim)

        pooled = self.ln(pooled)
        s = self.mlp(pooled).squeeze(-1)  # (B,)
        return s
...
def run_dataset(ds_name: str):
    ...
    for j, t in enumerate(train_targets):
        ...
        model = DeepMLPBagOfCodes(vocab_size=vocab_size, hidden_dim=EMB_DIM).to(DEVICE)
    ...
    print("\nTEST Metrics (DistCorr, MI_nats) using DeepMLP (bag-of-codes) 2-stage WEIGHTED scalar score s(x):")
