"""
DeepFM (Deep Factorization Machine) + Weighted Multi-Task BCE Clinical Outcome Predictor
=========================================================================================

Overview
--------
This script implements a DeepFM-based single-index model trained with binary
cross-entropy (BCE) on clinical admission data. The model operates on mean-pooled
ICD-10 token embeddings and combines a factorization machine (FM) interaction term
with a deep network (DNN) to produce a single scalar risk score. Training follows a
two-stage weighted multi-task strategy: Stage 1 trains per-target single-task models
to derive loss weights; Stage 2 trains one unified DeepFM on a normalized weighted
multi-task BCE objective. Checkpoint selection in both stages uses validation BCE.

Pipeline Flow
-------------
1.  Parse --data_root argument pointing to the final_datasets directory.
2.  Load train/val/test admission CSVs and diagnosis ICD-10 CSV.gz files in chunks
...
7.  Stage 2 — Weighted multi-task BCE training (10 epochs, AdamW, LR=1e-3):
        - Single DeepFM model trained on normalized weighted masked BCE
          (weighted sum of per-task BCE / weighted sum of valid counts).
        - Select best checkpoint by lowest weighted VAL BCE.
8.  Score the TEST split with the best Stage-2 DeepFM scalar output.
9.  Report Distance Correlation, and Mutual Information between DeepFM
    scores and each binary test label.

Architecture: DeepFMSingleIndex
--------------------------------
- Embedding layer for FM vectors (vocab_size → EMB_DIM=128), padding_idx=0
- Separate linear-term embedding (vocab_size → 1), padding_idx=0
- FM interaction: 0.5 * sum_k((sum_i v_i[k])^2 − sum_i v_i[k]^2), length-normalized
- Masked mean pooling of FM vectors → LayerNorm → deep tower
- Deep tower: Linear(128→128) → ReLU → Linear(128→128) → ReLU → Linear(128→1)
- Output: scalar logit = w0 + linear_term + FM_interaction + deep_term
...
Running
-------
    python DeepFM_mimiciv.py --data_root /path/to/final_datasets

Override training seed:
    SEED=42 python DeepFM_mimiciv.py --data_root /path/to/final_datasets

Example:
    python DeepFM_mimiciv.py --data_root ./data/final_datasets
"""
...
################################################################
# MODEL: DeepFMSingleIndex (ICD tokens -> scalar logit)
################################################################
class DeepFMSingleIndex(nn.Module):
    """
    DeepFM-style single-index model for a bag/multiset of ICD tokens.

    Given padded token ids x_ids (B,L), where PAD=0:
      logit = w0
            + sum_i w[token_i]
            + 0.5 * sum_k( (sum_i v_i[k])^2 - sum_i v_i[k]^2 )
            + DNN(mean_pool(v_i))

    Notes:
      - Treats each token occurrence as x_i=1 (so duplicates contribute multiple times).
      - Uses masking to ignore PAD.
      - Output is a single scalar logit per sample (B,).
    """
    def __init__(self, vocab_size: int, emb_dim: int):
        super().__init__()
        self.w0 = nn.Parameter(torch.zeros(1))

        # Linear term weights per token
        self.lin_emb = nn.Embedding(vocab_size, 1, padding_idx=0)

        # FM embedding vectors per token
        self.fm_emb = nn.Embedding(vocab_size, emb_dim, padding_idx=0)

        # Deep tower on pooled embedding
    ...
    def forward(self, x_ids: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        keep = (x_ids != 0).float()               # (B,L)
        keep_f = keep.unsqueeze(-1)               # (B,L,1)

        # ---------- Linear term ----------
        lin = self.lin_emb(x_ids).squeeze(-1)     # (B,L)
        lin = (lin * keep).sum(dim=1)             # (B,)

        # ---------- FM interaction term ----------
        v = self.fm_emb(x_ids) * keep_f           # (B,L,d)
        sum_v = v.sum(dim=1)                      # (B,d)
        sum_v2 = (v * v).sum(dim=1)               # (B,d)
        fm = 0.5 * ((sum_v * sum_v) - sum_v2)     # (B,d)
        fm = fm.sum(dim=1)                        # (B,)
        ...
        # >>> ADD HERE <<<
        den1 = den.squeeze(1)            # (B,)
        lin = lin / den1
        fm  = fm  / (den1 * den1)
        # >>> END ADD <<<

        pooled = sum_v / den                                # (B,d) mean pool
        pooled = self.pool_ln(pooled)
        deep = self.deep(pooled).squeeze(-1)                # (B,)

        return (self.w0 + lin + fm + deep)
...
def run_dataset(ds_name: str):
    ...
    for j, t in enumerate(train_targets):
        ...
        model = DeepFMSingleIndex(vocab_size=vocab_size, emb_dim=EMB_DIM).to(DEVICE)
    ...
    print("\nTEST Metrics (DistCorr, MI_nats) using DeepFM 2-stage WEIGHTED scalar score s(x):")
