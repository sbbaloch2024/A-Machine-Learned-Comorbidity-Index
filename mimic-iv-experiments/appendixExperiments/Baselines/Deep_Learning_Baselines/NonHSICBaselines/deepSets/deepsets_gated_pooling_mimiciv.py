"""
Deep Sets (Sigmoid Gating) + Weighted Multi-Task BCE Clinical Outcome Predictor
================================================================================

Overview
--------
This script implements a Deep Sets model with learned sigmoid gating
(DeepSetsSingleIndex) trained with binary cross-entropy (BCE) on clinical admission
data. Each admission's ICD-10 codes are embedded and transformed per-element via a
phi network. A token-wise gate MLP produces a scalar gate in (0, 1) for each token;
PAD gates are forced to zero. The gated weighted sum is normalized by total gate mass
and passed through a rho MLP to produce a single scalar risk score. Training follows
a two-stage weighted multi-task strategy: Stage 1 trains per-target single-task models
to derive loss weights; Stage 2 trains one unified model on a normalized weighted
...
Architecture: DeepSetsSingleIndex (Sigmoid Gating)
----------------------------------------------------
...
- Gate MLP (per-token scalar): Linear(128→128) → ReLU → Linear(128→1) → sigmoid;
  PAD token gates forced to 0 via keep mask
- Gated pooling: weighted sum of phi(E) by gate values, normalized by total gate
  mass (clamped to 1e-8 to handle empty admissions safely)
- LayerNorm applied to the gated pooled representation
...
Running
-------
    python deepsets_gated_pooling_mimiciv.py --data_root /path/to/final_datasets

Override training seed:
    SEED=42 python deepsets_gated_pooling_mimiciv.py --data_root /path/to/final_datasets

Example:
    python deepsets_gated_pooling_mimiciv.py --data_root ./data/final_datasets
"""
...
class DeepSetsSingleIndex(nn.Module):
    def __init__(self, vocab_size: int, emb_dim: int):
        ...
        # --- NEW: token-wise gate producing one scalar per token ---
    ...
    def forward(self, x_ids: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        ...
        keep = (x_ids != 0).float()      # (B, L), 1 for real tokens, 0 for PAD

        # gate logits per token -> gates in (0,1)
        gate_logits = self.gate_mlp(H).squeeze(-1)   # (B, L)
        gates = torch.sigmoid(gate_logits) * keep    # (B, L), PAD gates forced to 0

        # weighted sum over tokens
        pooled = torch.bmm(gates.unsqueeze(1), H).squeeze(1)  # (B, d)

        # normalize by total gate mass (handles empty sets safely)
