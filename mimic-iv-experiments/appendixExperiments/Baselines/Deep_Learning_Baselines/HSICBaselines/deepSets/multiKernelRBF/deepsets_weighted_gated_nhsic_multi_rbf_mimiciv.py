...
"""
DeepSets (Gated Pooling) + NHSIC Multi-Task Clinical Outcome Predictor
=======================================================================

Overview
--------
This script is a variant of the DeepSets + NHSIC pipeline that replaces
dot-product attention pooling with a learned sigmoid gate pooling mechanism.
Each token's contribution to the admission-level representation is scaled by
a sigmoid gate score, then normalized by the sum of gate weights (making the
pooled representation invariant to the number of codes). The rest of the
two-stage NHSIC training strategy, weight derivation, score orientation, and
evaluation protocol are identical to the attention-pooling variant.
...
Architecture: DeepSetsSingleScalar (Gated Pooling)
---------------------------------------------------
- Embedding layer (vocab_size → EMB_DIM=128)
- Per-token phi network: Linear → ReLU → Linear → ReLU
- Per-token sigmoid gate MLP: Linear → ReLU → Linear(1) → sigmoid
  (PAD tokens are forced to gate value 0)
- Gate-weighted sum pooled, normalized by total gate weight (division by
  clamped sum prevents NaN for empty admissions) → LayerNorm
- rho network: Linear → ReLU → Linear → ReLU → Linear(1) → scalar score

Difference from Attention-Pooling Variant
------------------------------------------
The previous script uses a dot-product attention query (softmax-normalized
weights) for pooling. This script uses sigmoid gates (unnormalized, then
explicitly divided by gate sum), which allows each token to be independently
gated rather than competing via softmax.
...
Running
-------
    python deepsets_weighted_gated_nhsic_multi_rbf_mimiciv.py --data_root /path/to/final_datasets

With optional arguments:
    python deepsets_weighted_gated_nhsic_multi_rbf_mimiciv.py \\
        --data_root /path/to/final_datasets \\
        --validity_mode intersection

Override training seed:
    SEED=42 python deepsets_weighted_gated_nhsic_multi_rbf_mimiciv.py --data_root /path/to/final_datasets

Example:
    python deepsets_weighted_gated_nhsic_multi_rbf_mimiciv.py --data_root ./data/final_datasets
"""
...
class DeepSetsSingleScalar(nn.Module):
    ...
    def forward(self, x_tokens: torch.Tensor, pad_mask: torch.Tensor) -> torch.Tensor:
        ...
        keep = (~pad_mask).float()                 # (B, L)  1 for real tokens, 0 for PAD

        logits = self.gate_mlp(H).squeeze(-1)      # (B, L)
        gates  = torch.sigmoid(logits) * keep      # (B, L) in [0,1], PAD -> 0

        # weighted sum (B,1,L) @ (B,L,d) -> (B,1,d) -> (B,d)
        pooled = torch.bmm(gates.unsqueeze(1), H).squeeze(1)  # (B, d)

        # normalize so scale doesn't depend on #codes (also avoids NaNs for empty admissions)
        den = gates.sum(dim=1, keepdim=True).clamp_min(1e-8)  # (B, 1)
        pooled = pooled / den
