"""
Deep Sets (Masked Sum Pooling) + Weighted Multi-Task BCE Clinical Outcome Predictor
=====================================================================================

Overview
--------
This script implements a Deep Sets model with masked sum pooling
...
summed across real non-PAD tokens to produce a single pooled
...
Architecture: DeepSetsSingleIndex (Masked Sum Pooling)
---------------------------------------------------------
...
- Masked sum pooling: sum of phi(E) over real tokens
...
Running
-------
    python deepsets_sum_pooling_mimiciv.py --data_root /path/to/final_datasets

Override training seed:
    SEED=42 python deepsets_sum_pooling_mimiciv.py --data_root /path/to/final_datasets

Example:
    python deepsets_sum_pooling_mimiciv.py --data_root ./data/final_datasets
"""
...
class DeepSetsSingleIndex(nn.Module):
    ...
    def forward(self, x_ids: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        ...
        pooled = H.sum(dim=1)               # masked sum pooling (B,d)
