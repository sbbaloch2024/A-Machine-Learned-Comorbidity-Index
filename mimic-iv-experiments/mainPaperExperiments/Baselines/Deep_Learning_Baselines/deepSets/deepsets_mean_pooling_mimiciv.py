"""
Deep Sets (Masked Mean Pooling) + Weighted Multi-Task BCE Clinical Outcome Predictor
=====================================================================================

Overview
--------
This script implements a Deep Sets model with masked mean pooling
...
phi network. PAD tokens are zeroed out and the remaining token representations are
averaged (sum divided by token count, clamped to 1e-8) to produce a single pooled
vector, which is then passed through a rho MLP to produce a scalar risk score.
Training follows a two-stage weighted multi-task strategy: Stage 1 trains per-target
single-task models to derive loss weights; Stage 2 trains one unified model on a
normalized weighted multi-task BCE objective. Checkpoint selection in both stages
uses validation BCE.
...
Architecture: DeepSetsSingleIndex (Masked Mean Pooling)
---------------------------------------------------------
...
- PAD tokens zeroed via keep mask before pooling
- Masked mean pooling: sum of phi(E) over real tokens / token count (clamped to 1e-8)
- LayerNorm applied to the pooled representation
...
Running
-------
    python deepsets_mean_pooling_mimiciv.py --data_root /path/to/final_datasets

Override training seed:
    SEED=42 python deepsets_mean_pooling_mimiciv.py --data_root /path/to/final_datasets

Example:
    python deepsets_mean_pooling_mimiciv.py --data_root ./data/final_datasets
"""
...
class DeepSetsSingleIndex(nn.Module):
    ...
    def forward(self, x_ids: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        ...
        H = H * keep.unsqueeze(-1)          # mask PAD tokens

        den = keep.sum(dim=1, keepdim=True).clamp_min(1e-8)  # (B,1)
        pooled = H.sum(dim=1) / den         # masked mean (B,d)
