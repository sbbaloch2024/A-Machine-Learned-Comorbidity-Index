...
data. Each admission's ICD-10 diagnosis codes are converted to 4-character prefix
...
3.  Load diagnosis ICD-10 CSV.gz files in chunks of 2M rows. Build a TRAIN-only
...
Running
-------
    python MILAttentionPooling_mimiciv.py --data_root /path/to/final_datasets

Override training seed:
    SEED=42 python MILAttentionPooling_mimiciv.py --data_root /path/to/final_datasets

Example:
    python MILAttentionPooling_mimiciv.py --data_root ./data/final_datasets
"""
...
class MILAttentionPool(nn.Module):
    ...
    def __init__(self, vocab_size: int, emb_dim: int, head_hidden: int = 128):
        ...
        # small post-pool head (can be 0-layer = Linear(emb_dim,1))
