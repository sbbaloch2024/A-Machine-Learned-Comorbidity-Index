...
the FM with a GBT trained on TF-IDF-weighted ICD-10 code bag-of-words features.
...
2.  Load train/val/test admission CSVs and diagnosis ICD-10 CSVs (gzipped).
...
Running
-------
    python gbt_mimiciv.py --data_root /path/to/final_datasets

Switching backend (XGBoost is the only fully implemented option):
    GBT_BACKEND=xgb python gbt_mimiciv.py --data_root /path/to/final_datasets

Example:
    python gbt_mimiciv.py --data_root ./data/final_datasets
"""
...
def load_diagnoses(ds_dir: Path, diag_stem: str, split: str) -> pd.DataFrame:
    ...
    # ICD-10 prefix tokenization (match DL scripts)
