...
data. Each admission's ICD-9 diagnosis codes are converted to fixed-length
...
2.  Load train/val/test admission CSVs and diagnosis ICD-9 CSV.gz files for the
    CORE dataset.
...
6.  Load diagnosis ICD-9 files in chunks of 2M rows and convert each ICD code to
    a normalized 4-character prefix token.
7.  Build a TRAIN-only vocabulary from ICD-9 prefix tokens:
        - PAD token has ID 0.
        - UNK token has ID 1.
...
- Graph structure:
      node 0 is CLS
      nodes 1..L are ICD-9 token nodes
...
Running
-------
    python starGAT_mimiciii.py --data_root /path/to/final_datasets

Override training seed:
    SEED=42 python starGAT_mimiciii.py --data_root /path/to/final_datasets

Example:
    python starGAT_mimiciii.py --data_root ./data/final_datasets
"""
