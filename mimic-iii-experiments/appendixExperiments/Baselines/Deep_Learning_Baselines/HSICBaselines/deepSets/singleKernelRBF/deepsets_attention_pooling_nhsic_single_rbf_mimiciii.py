...
"""
DeepSets (Dot-Product Attention Pooling) + Single-RBF NHSIC Multi-Task Clinical Outcome Predictor
===================================================================================================

Overview
--------
This script is a variant of the DeepSets + NHSIC pipeline that uses dot-product
attention pooling and a single-scale RBF kernel (rather than the multi-scale RBF
used in other variants). A learned query vector scores each token, softmax weights
are computed (PAD-masked and renormalized to handle empty admissions), and the
weighted sum forms the pooled representation. The NHSIC objective uses a single
RBF kernel with a median-heuristic sigma, making it simpler but less multi-scale
than the multi-RBF variants.
...
8.  Stage 2 — Weighted multi-task NHSIC training (10 epochs, AdamW, LR=1e-3):
        - Objective is the weighted sum of per-task single-RBF NHSIC values.
...
Architecture: DeepSetsSingleScalar (Dot-Product Attention Pooling)
-------------------------------------------------------------------
- Embedding layer (vocab_size → EMB_DIM=128)
- Per-token phi network: Linear → ReLU → Linear → ReLU
- PAD tokens zeroed out before attention scoring
- Dot-product attention: scores = (H · q) / sqrt(d), where q is a learned
  query parameter; PAD positions filled with -1e9 before softmax
- Softmax weights multiplied by keep mask and renormalized (handles empty sets)
- Weighted sum pooled → LayerNorm
- rho network: Linear → ReLU → Linear → ReLU → Linear(1) → scalar score

Key Difference from Multi-RBF Variants
----------------------------------------
This script uses a single RBF kernel (rbf_kernel_1d) with one sigma value,
whereas the multi-RBF variants average kernels at five scales
[0.25, 0.50, 1.00, 2.00, 4.00] × sigma_base. The single-RBF kernel is
simpler and faster but less robust to varying score spread.
...
Running
-------
    python deepsets_attention_pooling_nhsic_single_rbf_mimiciii.py --data_root /path/to/final_datasets

With optional arguments:
    python deepsets_attention_pooling_nhsic_single_rbf_mimiciii.py \\
        --data_root /path/to/final_datasets \\
        --validity_mode intersection

Override training seed:
    SEED=42 python deepsets_attention_pooling_nhsic_single_rbf_mimiciii.py --data_root /path/to/final_datasets

Example:
    python deepsets_attention_pooling_nhsic_single_rbf_mimiciii.py --data_root ./data/final_datasets
"""
...
class DeepSetsSingleScalar(nn.Module):
    def __init__(self, vocab_size: int, dim: int = EMB_DIM):
        ...
        # REPLACE gate_mlp with learnable attention query
        self.attention_query = nn.Parameter(torch.randn(1, 1, dim) / np.sqrt(dim))
    ...
    def forward(self, x_tokens: torch.Tensor, pad_mask: torch.Tensor) -> torch.Tensor:
        ...
        keep = (~pad_mask).float()      # (B, L)
        H = H * keep.unsqueeze(-1)      # zero PAD contributions (important)

        # scores (B, L)
        q = self.attention_query.squeeze(0).squeeze(0)           # (d,)
        scores = (H * q).sum(dim=-1) / math.sqrt(H.size(-1))     # dot-prod attention

        # mask PAD scores
        scores = scores.masked_fill(pad_mask, -1e9)

        # weights (B, L), then force PAD=0 and renormalize (handles empty sets)
...
def nhsic_on_split(
    ...
    for local_j, (t, j) in enumerate(zip(targets, target_indices)):
        ...
        K = rbf_kernel_1d(sv, sigma)
        L = delta_kernel(yv_t)
        val = nhsic_kernel_alignment(K, L).detach().float().cpu().item()
