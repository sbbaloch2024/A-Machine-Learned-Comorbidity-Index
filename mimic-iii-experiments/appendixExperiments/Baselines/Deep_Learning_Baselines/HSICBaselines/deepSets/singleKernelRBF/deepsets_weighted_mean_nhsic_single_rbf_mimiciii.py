"""
DeepSets (Masked Mean Pooling + LayerNorm) + Single-RBF NHSIC Multi-Task Clinical Outcome Predictor
=====================================================================================================

Overview
--------
This script is a variant of the DeepSets + NHSIC pipeline combining masked mean
pooling with LayerNorm and a single-scale RBF kernel. Token embeddings are averaged
over non-PAD positions (divided by clamped valid count), followed by LayerNorm
before the scalar output head. The NHSIC objective uses a single RBF kernel
(rbf_kernel_1d) via the hsic_normalized helper, with sigma estimated by the
median pairwise distance heuristic. This is the mean-pooling analogue of the
single-RBF gated and attention variants.
...
10. Evaluate on TEST: per-task NHSIC, Distance Correlation, and
    Mutual Information.

Architecture: DeepSetsSingleScalar (Masked Mean Pooling + LayerNorm)
---------------------------------------------------------------------
- Embedding layer (vocab_size → EMB_DIM=128)
- Per-token phi network: Linear → ReLU → Linear → ReLU
- PAD tokens zeroed out; masked mean pool: sum(H * keep) / clamp(count, min=1)
- LayerNorm on the pooled representation
- rho network: Linear → ReLU → Linear → ReLU → Linear(1) → scalar score

Key Difference from Other Variants
------------------------------------
Uses the same masked mean pooling + LayerNorm as the multi-RBF mean variant,
but the NHSIC kernel is single-scale (rbf_kernel_1d) rather than the average
of five RBF scales. The NHSIC normalization also uses a numerically stabilized
eps-inside-sqrt form (hsic_normalized) rather than clamp_min on the denominator.
...
class DeepSetsSingleScalar(nn.Module):
    ...
    def forward(self, x_tokens: torch.Tensor, pad_mask: torch.Tensor) -> torch.Tensor:
        ...
        H = H * keep
        valid_count = keep.squeeze(-1).sum(dim=1, keepdim=True).clamp(min=1)
        pooled = H.sum(dim=1) / valid_count
...
def hsic_normalized(K: torch.Tensor, L: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    Kc = center_kernel(K)
    Lc = center_kernel(L)
    num = (Kc * Lc).sum()
...
def nhsic_on_split(
    ...
    for local_j, (t, j) in enumerate(zip(targets, target_indices)):
        ...
        yy_full = Y[:, j].astype(np.int64)
        if np.unique(yy_full[valid]).size < 2:
            out[t] = 0.0
            continue
