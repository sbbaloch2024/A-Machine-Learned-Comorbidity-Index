"""
DeepSets (Sigmoid Gate Pooling, No LayerNorm) + Single-RBF NHSIC Multi-Task Clinical Outcome Predictor
========================================================================================================

Overview
--------
This script is a variant of the DeepSets + single-RBF NHSIC pipeline that uses
sigmoid gate pooling without a LayerNorm on the pooled representation. Each
token's contribution is weighted by a learned sigmoid gate score (PAD forced to
zero), the weighted sum is normalized by the total gate weight, and the result
feeds directly into the rho output head without any normalization layer. The
single-scale RBF kernel with median-heuristic sigma is used for all NHSIC
computations.
...
Architecture: DeepSetsSingleScalar (Sigmoid Gate Pooling, No LayerNorm)
------------------------------------------------------------------------
...
- Gate-weighted sum pooled, divided by clamped total gate weight
- No LayerNorm before the output head
- rho network: Linear → ReLU → Linear → ReLU → Linear(1) → scalar score

Difference from Other Variants
--------------------------------
This variant uses the same sigmoid gate pooling mechanism as the gated
multi-RBF variant, but omits the LayerNorm after pooling and uses a single-scale
RBF kernel instead of the five-scale multi-RBF. Removing LayerNorm means the
pooled representation scale is governed entirely by the gate normalization.
...
Running
-------
    python deepsets_weighted_mean_nhsic_single_rbf_mimiciii.py --data_root /path/to/final_datasets

With optional arguments:
    python deepsets_weighted_mean_nhsic_single_rbf_mimiciii.py \\
        --data_root /path/to/final_datasets \\
        --validity_mode intersection

Override training seed:
    SEED=42 python deepsets_weighted_mean_nhsic_single_rbf_mimiciii.py --data_root /path/to/final_datasets

Example:
    python deepsets_weighted_mean_nhsic_single_rbf_mimiciii.py --data_root ./data/final_datasets
"""
...
class DeepSetsSingleScalar(nn.Module):
    ...
    def forward(self, x_tokens: torch.Tensor, pad_mask: torch.Tensor) -> torch.Tensor:
        ...
        # normalize (avoid dependence on number of codes)
