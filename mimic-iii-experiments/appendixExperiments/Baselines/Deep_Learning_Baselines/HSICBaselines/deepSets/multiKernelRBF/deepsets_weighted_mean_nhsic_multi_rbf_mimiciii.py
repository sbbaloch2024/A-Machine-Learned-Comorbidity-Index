"""
DeepSets (Masked Mean Pooling + LayerNorm) + NHSIC Multi-Task Clinical Outcome Predictor
==========================================================================================

Overview
--------
This script is a variant of the DeepSets + NHSIC pipeline that uses masked mean
pooling with LayerNorm as the set aggregation mechanism. Each admission's ICD-9
token embeddings are averaged over non-PAD positions (divided by valid token count,
clamped to avoid division by zero), then passed through a LayerNorm before the
scalar output head. The two-stage NHSIC training strategy, weight derivation,
score orientation, and evaluation protocol are identical to the other DeepSets
variants.
...
Architecture: DeepSetsSingleScalar (Masked Mean Pooling)
---------------------------------------------------------
...
- Masked mean pool: sum(H * keep) / clamp(count(valid), min=1)
...
-----------------------------------------
The attention-pooling variant uses a learned dot-product query with softmax
normalization. The gated-pooling variant uses per-token sigmoid gates with
explicit normalization. This variant uses the simplest aggregation — unweighted
masked mean — stabilized by a LayerNorm, making it the most parameter-efficient
of the three pooling strategies.
...
Running
-------
    python deepsets_weighted_mean_nhsic_multi_rbf_mimiciii.py --data_root /path/to/final_datasets

With optional arguments:
    python deepsets_weighted_mean_nhsic_multi_rbf_mimiciii.py \\
        --data_root /path/to/final_datasets \\
        --validity_mode intersection

Override training seed:
    SEED=42 python deepsets_weighted_mean_nhsic_multi_rbf_mimiciii.py --data_root /path/to/final_datasets

Example:
    python deepsets_weighted_mean_nhsic_multi_rbf_mimiciii.py --data_root ./data/final_datasets
"""
...
class DeepSetsSingleScalar(nn.Module):
    def __init__(self, vocab_size: int, dim: int = EMB_DIM):
        ...
        # ✅ ADD: LayerNorm after masked mean pooling
    ...
    def forward(self, x_tokens: torch.Tensor, pad_mask: torch.Tensor) -> torch.Tensor:
        ...
        # Masked mean pooling: sum / count(valid)
        valid_counts = (~pad_mask).float().sum(dim=1, keepdim=True)  # (B, 1)
        valid_counts = torch.clamp(valid_counts, min=1.0)            # avoid div by 0
        pooled = H.sum(dim=1) / valid_counts                         # (B, d)

        # ✅ LayerNorm on pooled representation
