...
"""
DeepSets + NHSIC Multi-Task Clinical Outcome Predictor
=======================================================

Overview
--------
This script trains a DeepSets neural network with attention pooling to produce a
single scalar risk score from ICD-9 diagnosis code sets. The model is optimized
using Normalized Hilbert-Schmidt Independence Criterion (NHSIC) as the objective,
maximizing statistical dependence between the learned score and binary clinical
outcomes. A two-stage pipeline derives per-task weights from Stage-1 single-task
NHSIC models, then trains a weighted multi-task model in Stage 2.

The final score is sign-oriented on VAL using mortality as an anchor (higher
score = higher risk), then evaluated on TEST.

Pipeline Flow
-------------
1.  Parse --data_root (path to final_datasets) and --validity_mode
    ("taskwise" or "intersection") controlling which admissions are considered
    valid per NHSIC objective.
...
3.  Build hadm_id → token-list mappings for each split, capped at 256 codes per
    admission in file-encounter order.
4.  Construct AdmissionsSetDataset instances with per-target label matrices Y
    ...
    long_stay_defined rather than NaN.
5.  Estimate sigma_base for the multi-RBF kernel by computing the median pairwise
    absolute score difference over a subsample of up to 5000 TRAIN predictions
...
10. Evaluate on TEST: per-task NHSIC (subsampled),  Distance Correlation,
...
- Dot-product attention pooling with a learned query vector and LayerNorm
...
delta (equality) kernel over binary labels. NHSIC = normalized Frobenius alignment
between centered score and label kernels.
...
Running
-------
    python deepsets_attention_pooling_nhsic_multi_rbf_mimiciii.py --data_root /path/to/final_datasets

With optional arguments:
    python deepsets_attention_pooling_nhsic_multi_rbf_mimiciii.py \\
        --data_root /path/to/final_datasets \\
        --validity_mode intersection

Override training seed:
    SEED=42 python deepsets_attention_pooling_nhsic_multi_rbf_mimiciii.py --data_root /path/to/final_datasets

Example:
    python deepsets_attention_pooling_nhsic_multi_rbf_mimiciii.py --data_root ./data/final_datasets
"""
