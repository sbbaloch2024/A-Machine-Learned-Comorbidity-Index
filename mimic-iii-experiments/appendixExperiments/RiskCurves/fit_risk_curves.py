...
------------------------
This script expects the following MIMIC-III diagnostic CSVs in the sibling
`DiagnosticCSV/` directory:
...
Generate binned and isotonic risk curves for each seed
------------------------------------------------------
...
for task in TASKS:
    d = df[df["task"] == task]
    label, delta = label_from_curve(d)
    entries.append(f"{label} (\\(\\Delta={delta:.3f}\\))")

print(f"Seed {seed}:")
print("\\textbf{MIMIC-III} & " + " & ".join(entries) + " \\\\")
PY
    done
...
all_deltas = {task: [] for task in TASKS}

for seed in SEEDS:
    df = pd.read_csv(f"seed{seed}/curves_isotonic.csv")
    for task in TASKS:
        d = df[df["task"] == task]
        all_deltas[task].append(delta_from_curve(d))

entries = []
for task in TASKS:
    values = np.asarray(all_deltas[task], dtype=float)
    mean = values.mean()
    sd = values.std(ddof=1)
    label = label_from_delta(mean)
    entries.append(f"{label} (\\(\\Delta={mean:.3f}\\pm{sd:.3f}\\))")

print("\\textbf{MIMIC-III} & " + " & ".join(entries) + " \\\\")
print()
print("Raw deltas:")
for task in TASKS:
    print(task, [round(value, 3) for value in all_deltas[task]])
PY
...
Notes
-----
- This script uses relative paths only.
- Each seed is processed independently.
- Output files are saved under `seed11/`, `seed101/`, and `seed1001/`.
