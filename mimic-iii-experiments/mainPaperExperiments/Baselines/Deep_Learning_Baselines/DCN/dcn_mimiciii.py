...
"""
DCN (Deep & Cross Network) + Weighted Multi-Task BCE Clinical Outcome Predictor
================================================================================

Overview
--------
This script replaces the NHSIC objective and neural set encoder with a Deep &
Cross Network (DCN) trained directly on binary cross-entropy. The DCN operates
on mean-pooled ICD-9 token embeddings (masked, LayerNorm-stabilized) and
combines a cross network (explicit feature interactions) with a deep network
(nonlinear transformations) to produce a single scalar risk score. Training
follows the same two-stage weighted multi-task strategy as the NHSIC variants,
but uses BCE loss and val BCE for weight derivation and checkpoint selection.
...
2.  Load train/val/test admission CSVs and diagnosis ICD-9 CSV.gz files in
    ...
    (PAD=0, UNK=1) from per-admission token lists (duplicates kept, order preserved).
...
4.  Build label matrices Y (N, T) and validity masks M (N, T); long_stay
    validity driven by long_stay_defined. Zero out masks for single-class
    TRAIN targets before Stage 2 (both TRAIN and VAL masks).
5.  Stage 1 — Per-target single-task BCE training (10 epochs, AdamW, LR=1e-3):
        - Fresh DCN model per target.
...
7.  Stage 2 — Weighted multi-task BCE training (10 epochs, AdamW, LR=1e-3):
        - Single DCN model trained on normalized weighted masked BCE
          (weighted sum of per-task BCE / weighted sum of valid counts).
        - Select best checkpoint by lowest weighted VAL BCE.
8.  Score the TEST split with the best Stage-2 DCN scalar output.
9.  Report  Distance Correlation, and Mutual Information between DCN
    scores and each binary test label.

Architecture: DCNSingleIndex
-----------------------------
- Embedding layer (vocab_size → EMB_DIM=128), padding_idx=0
- Masked mean pooling: sum(E * keep) / clamp(count, min=1), followed by LayerNorm
- Cross network (2 layers): each CrossLayer computes
    x_{l+1} = x_l + x0 * (x_l · w_l) + b_l
  where x0 is the pooled embedding and w_l, b_l are learned parameters.
- Deep network (2 layers): Linear(128→128) → ReLU → Linear(128→128) → ReLU
- Head: Linear(EMB_DIM + 128 → 1) on concatenated cross and deep outputs → scalar

Key Configuration
-----------------
  SEED: controlled via SEED env var (default 1001); EVAL_SEED=12345 (fixed)
  EMB_DIM=128, NUM_CROSS_LAYERS=2, DEEP_HIDDEN=(128, 128), DROPOUT=0.0
  BATCH_SIZE=256, LR=1e-3, WEIGHT_DECAY=0.0, EPOCHS=10
...
Running
-------
    python dcn_mimiciii.py --data_root /path/to/final_datasets

Override training seed:
    SEED=42 python dcn_mimiciii.py --data_root /path/to/final_datasets

Example:
    python dcn_mimiciii.py --data_root ./data/final_datasets
"""
...
################################################################
# MODEL: DCNSingleIndex (Deep & Cross Network over pooled ICD embedding)
################################################################

class CrossLayer(nn.Module):
    """
    Classic DCN cross layer:
      x_{l+1} = x_l + x0 * (x_l @ w_l) + b_l
    where w_l is (d,), b_l is (d,)
    """
    ...
    def forward(self, x_l: torch.Tensor, x0: torch.Tensor) -> torch.Tensor:
        # x_l, x0: (B, d)
        # (B,) = sum over d of x_l * w
        xlw = (x_l * self.w).sum(dim=1, keepdim=True)  # (B,1)
        return x_l + x0 * xlw + self.b  # broadcast b: (d,)


class DCNSingleIndex(nn.Module):
    def __init__(
        self,
        vocab_size: int,
        emb_dim: int,
        num_cross_layers: int = 2,
        deep_hidden: tuple[int, ...] = (128, 128),
        dropout: float = 0.0,
    ):
        super().__init__()
        self.emb = nn.Embedding(vocab_size, emb_dim, padding_idx=0)

        # same pooling stabilization idea as DeepSets
        self.pool_ln = nn.LayerNorm(emb_dim)

        # Cross net
        self.cross = nn.ModuleList([CrossLayer(emb_dim) for _ in range(num_cross_layers)])
        ...
        # Combiner -> scalar
        deep_out_dim = deep_hidden[-1] if len(deep_hidden) > 0 else emb_dim
        self.head = nn.Linear(emb_dim + deep_out_dim, 1)

    def forward(self, x_ids: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        # x_ids: (B,L) padded with 0
        E = self.emb(x_ids)                      # (B,L,d)
        keep = (x_ids != 0).float()              # (B,L)
        E = E * keep.unsqueeze(-1)               # mask PADs

        den = keep.sum(dim=1, keepdim=True).clamp_min(1.0)  # (B,1)
        x0 = E.sum(dim=1) / den                  # pooled mean embedding (B,d)
        x0 = self.pool_ln(x0)
        ...
        # Deep tower
        d = self.deep(x0)                        # (B,deep_out)

        # Combine and score
        z = torch.cat([x, d], dim=1)             # (B, d + deep_out)
...
def run_dataset(ds_name: str):
    ...
    print("\nTEST Metrics (DistCorr, MI_nats) using DCN 2-stage WEIGHTED scalar score s(x):")
