...
or Complement NB — on TF-IDF-weighted ICD-9 code bag-of-words features, converts
...
Running
-------
    python nb_mimiciii.py --data_root /path/to/final_datasets

Switch NB variant at runtime:
    NB_KIND=multinomial python nb_mimiciii.py --data_root /path/to/final_datasets

Example:
    python nb_mimiciii.py --data_root ./data/final_datasets
"""
...
NB_KIND = os.environ.get("NB_KIND", "complement").strip().lower()
if NB_KIND not in {"multinomial", "complement"}:
    raise ValueError("NB_KIND must be 'multinomial' or 'complement'")
print(NB_KIND)
NB_ALPHA = 1.0            # Laplace smoothing
